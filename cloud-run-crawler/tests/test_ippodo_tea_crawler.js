/**
 * Test file for Ippodo Tea crawler
 * Tests product extraction, image processing, and Firebase integration
 */

const IppodoTeaCrawler = require('../src/crawlers/ippodo-tea-crawler');

async function testIppodoTeaCrawler() {
    console.log('=== Testing Ippodo Tea Crawler ===\n');
    ...
    const crawler = new IppodoTeaCrawler(testLogger);
    
    try {
        // Test URL accessibility
        console.log('1. Testing site accessibility...');
        const testResponse = await fetch('https://global.ippodo-tea.co.jp/collections/matcha');
    ...
    console.log('\n=== Ippodo Tea Crawler Test Complete ===');
}

// Run the test
testIppodoTeaCrawler().catch(console.error);