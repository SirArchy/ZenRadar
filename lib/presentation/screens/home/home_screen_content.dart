...
import 'package:zenradar/data/services/analytics/search_history_service.dart';
import 'package:zenradar/data/services/analytics/recommendation_service.dart';
import 'package:zenradar/data/services/backend/backend_service.dart';
...
import 'package:zenradar/data/services/cache/image_cache_manager.dart';
import 'package:zenradar/data/services/media/image_url_processor.dart';
import 'package:zenradar/presentation/widgets/filters/product_filters.dart';
import 'package:zenradar/presentation/widgets/filters/mobile_filter_modal.dart';
...
import 'package:zenradar/presentation/widgets/tutorial/swipe_tutorial_overlay.dart';
import 'package:zenradar/presentation/screens/product/product_detail_page.dart';
...
class _HomeScreenContentState extends State<HomeScreenContent>
  ...
  void _onScroll() {
    ...
    if (currentPosition >= maxExtent * 0.8) {
      _loadProducts(loadMore: true);
    }

    // Close search suggestions and filter section when scrolling
    ...
    // If we already updated state, don't continue with animation logic
    if (needsUpdate) {
    ...
    // Detect scroll direction and auto-hide/show filter section
    if (currentPosition > _lastScrollPosition && currentPosition > 150) {
      // Scrolling down - hide filter section with delay
      if (!_isScrollingDown) {
        _isScrollingDown = true;
        _hideFilterTimer = Timer(const Duration(milliseconds: 500), () {
          _hideFilterSection();
        });
      }
    } else if (currentPosition < _lastScrollPosition) {
      // Scrolling up - show filter section immediately
      if (_isScrollingDown) {
        _isScrollingDown = false;
        _showFilterSectionAnimated();
      }
    }
  ...
  void _showFilterSectionAnimated() {
    if (_showFilterSection && _isFilterSectionExpanded) return;
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () {
        ...
        if (_isFilterSectionExpanded && _showFilterSection) {
          setState(() {
            _isFilterSectionExpanded = false;
          });
          _filterAnimationController.reverse();
        ...
        children: [
          Column(
            children: [
              ...
              if (_showFilterSection && _isFilterSectionExpanded)
                SlideTransition(
                  position: _filterSlideAnimation,
                  child: FadeTransition(
                    opacity: _filterOpacityAnimation,
                    child: _buildCollapsibleFilterContent(),
                  ),
                ),
